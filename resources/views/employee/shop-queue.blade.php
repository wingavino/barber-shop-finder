...
$(document).ready(function(){
  function updateCurrentTicket(){
    $.ajax({
      url:'/queue/{{ App\Models\Employee::where('user_id', Auth::user()->id)->first()->shop->queue->id }}/current_ticket',
      type:'GET',
      dataType:'json',
                      ...
                      <div class="card">
                        ...
                        <div class="card-footer">
                          <div class="row text-center">
                            <div class="col">
                              <form method="POST" action="{{ route('employee.shop.queue.hold') }}">
                                @csrf
                                <button class="btn btn-primary" type="submit" name="button">Put on Hold</button>
                              </form>
                            </div>
                            <div class="col">
                              <form method="POST" action="{{ route('employee.shop.queue.next.hold') }}">
                                @csrf
                                <button class="btn btn-primary" type="submit" name="button">Next from On Hold</button>
                              </form>
                            </div>
                            <div class="col">
                              <form method="POST" action="{{ route('employee.shop.queue.finish') }}">
                                @csrf
                                ...
                                <div class="card-body text-center">
                                  <div class="row">
                                    ...
                                    @else
                                      ...
                                      <div class="col">
                                        <form method="POST" action="{{ route('employee.shop.queue.next', ['id' => $on_hold_ticket->id]) }}">
                                          @csrf
                                    ...
                                    <form method="POST" action="">
                                      ...
                                      <h3>
                                        @isset($shop->queue->next_ticket)
                                          <button class="btn btn-danger disabled" id="next_ticket" type="button" name="button">{{ $shop->queue->next_ticket }}</button>
                                        @else
                                          <button class="btn btn-danger disabled" type="button" name="button">None</button>
                                        @endisset
                                      </h3>
